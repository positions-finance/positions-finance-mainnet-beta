...
contract PositionsUniV3Handler is UUPSUpgradeable, AccessControlUpgradeable {
    ...
    error PositionsUniV3Handler__ArrayLengthMismatch();
    error PositionsUniV3Handler__InvalidWithdrawStatus();
        ...
        onlyRole(RELAYER_ROLE)
    {
        if (_requestIds.length != _statuses.length) {
            revert PositionsUniV3Handler__ArrayLengthMismatch();
        }
        ...
        for (uint256 i; i < _requestIds.length; i++) {
            ...
            if (status == Status.ACCEPTED) {
                ...
                if (withdrawalData.status != Status.PENDING) {
                    revert PositionsUniV3Handler__InvalidWithdrawStatus();
                }
                userNfts[withdrawalData.tokenId].remove(withdrawalData.poolOrVault);
                delete nftOwners[withdrawalData.poolOrVault];
            }
    ...
    /// @notice Gets all users deposited nfts.
    /// @param _tokenId The user nft tokenId.
    function getUserNfts(uint256 _tokenId) external view returns (uint256[] memory) {
        return userNfts[_tokenId].values();
    }
}
